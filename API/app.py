...
def create_app():
    ...
    # Initialize DB and fill products
    init_db()
    ...
    def index():
        """
        Main shop page with pagination, cart summary and categories.
        """
    ...
    def register():
        ...
        if request.method == 'POST':
            ...
            if auth_service.password_check(password):
                ...
                flash('Your password does not meet security conditions. '
                      'At least 8 characters, 1 lowercase, 1 uppercase, 1 digit, 1 special symbol.')
    ...
    def update_cart_product_quantity():
        """
        Increment or decrement quantity of a product in the cart.
        """
        product_id = request.form.get('product_id')
    ...
    def checkout():
        """
        Finalize the order, send confirmation email, and update product quantities.
        """
        if not AuthService.is_logged_in():
            return redirect(url_for('auth'))
        ...
        for cart_id in cart_ids:
            ...
            product_repo.update_product(
                cart_id, None, None, None, product.quantity - cart_ids[cart_id], None
            )
    ...
    def search():
        """
        Perform a fuzzy product name search and display matching results.
        """
        ...
        matched_products = [product for product in products if product.name in matches]
    ...
    def search_by_cat():
        ...
        for category_name in request.form:
            ...
            if products_by_cat:
                products_by_cats.extend(products_by_cat)
